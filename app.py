...
def prepare_dataset(path = 'dumps/netaporter_gb.json'):
  ...
  json_ = pd.read_json('dumps/netaporter_gb.json', lines=True,chunksize = 50)
  for chunk in json_:
      json_data = json_data.append(chunk) 
...
def higher_price_list(json_data):
  ...
  for prod in range(0,len(json_data)):
    ...
    dump_v = json.dumps(v_price_dict)
    price_json_obj_v = json.loads(dump_v)
    get_nap_offer_price = price_json_obj_v.get('offer_price')
...
def Process_inp():
    ...
    return jsonify(discounted_products_list=output)
